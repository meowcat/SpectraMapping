...
.write_massbank_spectrum <- function(spectrum_id, variables, peaks, backend) {
  ...
  if(!("NUM_PEAKS" %in% names(var_print))) {
    warning(glue("NUM_PEAKS missing for record {spectrum_id}. Recalculating"))
    var_render <- c(var_render, glue("  NUM_PEAKS: {nrow(peaks)}"))
  }
...
.massbank_writer <- function() {
  .massbank_writer_ <- function(backend) {
    ...
    file <- data %>% pmap_chr(.write_massbank_spectrum, backend = backend)
    return(file)
  }
  .massbank_writer_
}
